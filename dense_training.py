"""Training utilities for the dense Arc-Standard M2 experiment."""
...
def build_transition_inventory(labels):
    """Build the full Arc-Standard transition label space."""
    transitions = ["SHIFT"]
    for label in labels:
...
def generate_arc_standard_examples(sentences, transition_system, oracle):
    """Generate gold training examples from projective sentences."""
    examples = []
            ...
            examples.append(
                {
                    ...
                    "s1_idx": config.get_stack_second(),
                    "s0_idx": config.get_stack_top(),
                    "b0_idx": config.get_buffer_front(),
                    "b1_idx": config.get_buffer_second(),
...
def train_dense_model(
    ...
    """Train the dense transition classifier with mini-batches."""
    transition_inventory = build_transition_inventory(labels)
    train_examples, transition_counts = generate_arc_standard_examples(
        train_sentences,
        transition_system,
        oracle,
    )
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        for start_idx in range(0, len(shuffled_examples), batch_size):
            ...
            batch_features = [
                feature_extractor.extract_from_positions(
            ...
            batch_labels = [label_to_idx[item["gold_transition"]] for item in batch]

            inputs = torch.as_tensor(np.stack(batch_features), dtype=torch.float32, device=model.device)
            targets = torch.as_tensor(batch_labels, dtype=torch.long, device=model.device)

"""Train and document the M3 Arc-Standard contextual-rich experiment."""
...
from contextual_rich_training import train_dense_rich_model
...
DEFAULT_EXPERIMENT_DIR = PROJECT_ROOT / "experiments" / "m3_arc_standard_contextual_rich"
DEFAULT_CONFIG_PATH = DEFAULT_EXPERIMENT_DIR / "config.json"


def main():
    arg_parser = argparse.ArgumentParser(description="Run the M3 Arc-Standard contextual-rich experiment")
...
def run_experiment(config, experiment_dir, figures_dir):
    print("=" * 80)
    print(f"Running {config['model_id']} Arc-Standard contextual-rich experiment")
    print("=" * 80)
    ...
    device = "cuda" if config["contextual"]["use_gpu_if_available"] and _torch_cuda_available() else "cpu"
    embedding_provider = CamembertEmbeddingProvider(
        model_name=config["contextual"]["model_name"],
        device=device,
        pooling=config["contextual"]["pooling"],
        local_files_only=config["contextual"]["local_files_only"],
    )
    feature_extractor = ContextualRichFeatureExtractor(
    ...
    print(f"Train sentences kept for M3: {len(train_sentences)} / {len(raw_train_sentences)}")
    ...
    print(f"Contextual model:{config['contextual']['model_name']}")
    print(f"Hidden size:     {feature_extractor.word_embedding_dim}")
    print(f"Pooling:         {config['contextual']['pooling']}")
    print(f"Device:          {device}")
    print()

    precompute_sentence_embeddings(embedding_provider, train_sentences, "train")
    precompute_sentence_embeddings(embedding_provider, dev_sentences, "dev")
    precompute_sentence_embeddings(embedding_provider, test_sentences, "test")
    print()

    model = DenseRichTransitionClassifier(
        transition_classes=build_transition_classes(labels),
        word_embedding_dim=feature_extractor.word_embedding_dim,
    ...
    if best_checkpoint is None:
        raise RuntimeError("No best-dev checkpoint was saved for M3")
    ...
    results.update(
        {
            ...
            "feature_set_description": (
                "Frozen CamemBERT contextual word vectors for rich parser positions, "
                "plus POS embeddings, dependency-label embeddings, distance/stack/buffer "
                "structural buckets, and NULL/ROOT indicators"
            ),
            ...
            "contextual_model_name": config["contextual"]["model_name"],
            "contextual_hidden_size": feature_extractor.word_embedding_dim,
            "frozen_contextual_encoder": config["contextual"]["frozen"],
            "subword_pooling": config["contextual"]["pooling"],
            "contextual": config["contextual"],
            "pos_vocab_size": len(pos_vocab),
            "label_vocab_size": len(label_vocab),
            "cache_sizes": {
                "sentence_embeddings": embedding_provider.cache_size(),
            },
            "paths": {
    ...
    write_m3_readme(results, config, experiment_dir)

    print()
    print("Final M3 results (best-dev checkpoint)")
...
def write_m3_readme(results, config, experiment_dir):
    lines = [
        ...
        "## Contextual setup",
        "",
        f"- Contextual model: `{config['contextual']['model_name']}`",
        f"- Pooling strategy: `{config['contextual']['pooling']}`",
        ...
        "## Final results",
        "",
        f"- Projective training sentences kept: `{results['train_sentences_kept']} / {results['train_sentences_original']}`",
        f"- Generated transition examples: `{results['generated_transition_examples']}`",
        ...
        f"- Training time: `{results['training_time_seconds']:.2f}s`",
        f"- Parsing time: `{results['parsing_time_seconds']:.2f}s`",
        "",
        "## Outputs",
        "",
        "- `config.json`",
...
def precompute_sentence_embeddings(embedding_provider, sentences, split_name, report_every=250):
    """Warm the contextual cache with visible progress before training starts."""
    total = len(sentences)
    print(f"Precomputing CamemBERT embeddings for {split_name}: {total} sentences")
    start_time = time.time()
    for idx, sentence in enumerate(sentences, start=1):
        embedding_provider.get_sentence_embeddings(sentence)
        if idx % report_every == 0 or idx == total:
            elapsed = time.time() - start_time
            print(f"  {split_name}: cached {idx}/{total} sentences [{elapsed:.1f}s]")
